...
func MakeNewMyLog(fdir string, fname string, max_size int64, max_file int) *MyLog {
	...
	if err != nil {
		fmt.Printf("MakeNewMyLog Error:%s", err.Error())
		return nil
	}
	mf.state = FileOpen
	return mf
...
func (this *MyLog) putLine(buf string) bool {
	...
	if err != nil {
		fmt.Printf("putLine(%s) Error:%s", buf, err.Error())
		return false
	}
...
func (this *MyLog) PutLineAsLog(buf string) bool {
	if this != nil {
		...
		fmt.Println("[" + GetNowTime() + "]" + " " + src + " " + buf)
		return this.PutLine("[" + GetNowTime() + "]" + " " + buf)
	}
	return false
}
...
func (this *MyLog) checkBak() bool {
	//fmt.Println("checkBak1")
	if this.state == FileClose {
		return false
	}
	//fmt.Println("checkBak2")
	//检查文件是否已经超长
	if FileSize(this.Path) < this.MaxSize {
		return true
	}
	//fmt.Println("checkBak3")
	/*超长处理*/
	//关闭当前文件
	this.F.Close()
	//fmt.Println("checkBak4")
	this.F = nil
	var err error
	if this.MaxFiles <= 1 { //只允许一个文件
		...
		if err != nil {
			fmt.Printf("CheckBak Error:%s", err.Error())
			return false
		}
		return true
	}
	//fmt.Println("checkBak5")
	//多个文件就切换最老的文件
	flag := false
	i := 1
	for ; i < this.MaxFiles; i++ { //查找最旧日志名
		//fmt.Printf("checkBak51:%s\n", this.Path+".bak_"+I2s(i))
		if IsFile(this.Path + ".bak_" + I2s(i)) {
			//fmt.Println("checkBak52")
			continue
		} else {
			//fmt.Println("checkBak53")
			flag = true
			break
		}
		//fmt.Println("checkBak54")
	}
	//fmt.Println("checkBak6")
	if flag == true {
