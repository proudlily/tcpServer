...
// CreateFile 创建文件
func CreateFile(fdir string, fname string) *os.File {
	if !IsExist(GetExePath() + fdir) { //文件夹不存在，就地创建
		MkDir(GetExePath() + fdir)
	}
	fp, err := os.OpenFile(GetExePath()+fdir+"/"+fname, os.O_CREATE|os.O_APPEND|os.O_RDWR, 0666) //打开文件，不存在就创建，添加模式打开，读写打开
	if err != nil {
		fmt.Printf("GetNowFilePath Error:%s", err.Error())
		return nil
	}
	return fp
}
